...
/**
 * Modulation Config behavior from Stratum based Cassisni box. (gNMI + OpenConfig + TAI)
 */
public class StratumModulationConfig<T>
        extends AbstractGrpcHandlerBehaviour<GnmiClient, GnmiController>
        implements ModulationConfig<T> {

    public static Logger log = LoggerFactory.getLogger(StratumModulationConfig.class);
    ...
    public StratumModulationConfig() {
    ...
    public Optional<ModulationScheme> getModulationScheme(PortNumber portNumber, T component) {
        ...
        try {
            ...
            log.warn("Unable to get operational mode from device {}, port {}: {}", deviceId, portNumber, e.getMessage());
            return Optional.empty();
        }
