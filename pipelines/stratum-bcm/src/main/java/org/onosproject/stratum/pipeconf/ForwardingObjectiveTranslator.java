...
public class ForwardingObjectiveTranslator extends AbstractObjectiveTranslator<ForwardingObjective> {
    ...
    private void bridgingRule(ForwardingObjective obj, Set<Criterion> criteriaWithMeta,
        ...
        NextObjective nextObj = bcmObjectiveStore.getStoredNextObjective(obj.nextId());
        ...
        if (treatment == null) {
            throw new BcmPipelinerException(format("Unable to find next treatment for l2 unicast objective %s", obj.toString()));
        }

        PortNumber outputPort = BcmPipelineUtils.outputPort(treatment);
        if (outputPort == null) {
            throw new BcmPipelinerException(format("Unable to find output port from treatment %s", treatment));
        }
